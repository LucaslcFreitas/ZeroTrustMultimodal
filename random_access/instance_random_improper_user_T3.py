...
def getRandomImproperUserT3(min):
...
def createInstance(userInd, deviceInd, min, baseTime, resourcesPath):
    ...
    # Realiza o registro
    operations.append(getRegistry(userData[userInd], deviceData[deviceInd], baseTime, network, latitude, longitude))
    baseTime = baseTime + datetime.timedelta(minutes=1)

    # Realiza o login
    operations.append(getLogin(userData[userInd], deviceData[deviceInd], baseTime, network, latitude, longitude))
    baseTime = baseTime + datetime.timedelta(minutes=1)
    ...
    if len(resources['veryLowSensivity']) > 0:
        for i in range(15):
            ...
            operations.append(getAccess(userData[userInd], deviceData[deviceInd], baseTime, resources['veryLowSensivity'][ind], network, latitude, longitude))
            baseTime = baseTime + datetime.timedelta(minutes=1)
    ...
    if len(resources['lowSensivity']) > 0:
        for i in range(15):
            ...
            operations.append(getAccess(userData[userInd], deviceData[deviceInd], baseTime, resources['lowSensivity'][ind], network, latitude, longitude))
            baseTime = baseTime + datetime.timedelta(minutes=1)
    ...
    if len(resources['mediumSensivity']) > 0:
        for i in range(15):
            ...
            operations.append(getAccess(userData[userInd], deviceData[deviceInd], baseTime, resources['mediumSensivity'][ind], network, latitude, longitude))
            baseTime = baseTime + datetime.timedelta(minutes=1)
    ...
    if len(resources['highSensivity']) > 0:
        for i in range(5):
            ...
            operations.append(getAccess(userData[userInd], deviceData[deviceInd], baseTime, resources['highSensivity'][ind], network, latitude, longitude))
            baseTime = baseTime + datetime.timedelta(minutes=1)
    else:
        for i in range(5):
            ind = random.randint(0, len(resources['mediumSensivity'])-1)
    ...
    for i in range(10):
        ...
        operations.append(getAccess(userData[userInd], deviceData[deviceInd], baseTime, resources[ind], network, latitude, longitude))
        baseTime = baseTime + datetime.timedelta(minutes=1)

    # Troca os sinais do usuário, dispositivo, rede e local, simulando intrusão
    operations.append(getChangeSignals(userData[userInd]['REGISTRY']))
    deviceInd = changeDevice()
    network = '169.254.0.0/16'
    latitude = -23.5619368
    longitude = -46.7318469

    if min > 60:
        for i in range(min - 60):
            ind = random.randint(0, len(resources)-1)
            operations.append(getAccess(userData[userInd], alternativeDevice[deviceInd], baseTime, resources[ind], network, latitude, longitude))
            baseTime = baseTime + datetime.timedelta(minutes=1)

    return operations


def getRegistry(user, device, time, network, lat, long):
    latitude, logitude = add_variation_location(lat, long)
    return {
        "TYPE": "REGISTRY",
...
def getLogin(user, device, time, network, lat, long):
    latitude, logitude = add_variation_location(lat, long)
    return {
        "TYPE": "LOGIN",
        "REGISTRY": user["REGISTRY"],
...
def getAccess(user, device, time, access, network, lat, long):
    latitude, logitude = add_variation_location(lat, long)
    return {
        "REAUTHENTICATE": True,
        "TYPE": "ACCESS",
